...
<nav aria-label="Page navigation example" style="position: absolute;bottom: 10px;left: 42%">
    <ul class="pagination justify-content-center">
        <li class="page-item <c:if test="${pageInfo.pageNum==1}">disabled</c:if> ">
            <a class="page-link" href="/admin/article/list?page=1" >首页</a>
        </li>
        <li class="page-item <c:if test="${pageInfo.pageNum < 2}">disabled</c:if> ">
            <a class="page-link" href="/admin/article/list?page=${pageInfo.prePage}" >上一页</a>
        </li>
        ...
        <li class="page-item <c:if  test="${pageInfo.pageNum > (pageInfo.pages-1)}">disabled</c:if> ">
            <a class="page-link" href="/admin/article/list?page=${pageInfo.nextPage}" >下一页</a>
        </li>
        <li class="page-item  <c:if test="${pageInfo.pageNum==pageInfo.pages}">disabled</c:if> ">
            <a class="page-link" href="/admin/article/list?page=${pageInfo.pages}">最后一页</a>
        </li>
    </ul>
</nav>
...
<script>
    function fullScreen(title,url){
        /*调用layer.open方法，type为0（信息框，默认）1（页面层）2（iframe层）3（加载层）4（tips层），ifreame类似弹窗
            content内容任意，一般为使用iframe时，用来传递url、数组等各类信息,area为['500px', '300px']此类型，表示宽高。
            maxmin最大最小化按钮，只对type为1 2的有效 ，skin: 'demo-class'，在此open中添加样式，属性繁多。自己去看
        */
    ...
    function ifdelete(id,title) {
        //layer.confirm询问，如果为true  则会执行后面的function
